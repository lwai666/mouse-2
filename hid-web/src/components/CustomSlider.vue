...
<template>
  <div ref="containerRef" class="custo-slider-container">
    ...
    <ul
      ...
      :class="vertical ? `left-[-100%]` : 'bottom-[26px] -translate-x-1/2'"
      style="color: #DAFF00;"
    ...
    >
      ...
      <li class="cursor-pointer text-base" @dblclick="doubleClickEdit ? editValue() : null">
        <input v-if="isEditing" ref="inputRef" v-model="inputValue" class="w-[70px] text-center" type="text" @blur="saveValue" @keyup.enter="saveValue">
        <template v-else-if="labelTransform">
          {{ labelTransform(value) }}
        </template>
        <template v-else>
          {{ value }}{{ getUnit(defaultSelectOptions[0].label) }}
        </template>
      </li>
    </ul>
  </div>
