...
async function getBattery() {
  if (getBatteryFlag) {
    let flag = await getOnlineNoDialog()
...
function bufferToColor(buffer, index) {
  let color = `#${buffer[index].toString(16).padStart(2, '0')
...
async function updateDeviceInfo() {
  ...
  for (var i = 0; i < 8; i++) {
    var addr = i * 4 + 0x0C
    let high = (flashData[addr + 2] & 0x0C) >> 2
  ...
  if (mouseCfg.init == false) {
    ...
    for (var i = 0; i < 16; i++) {
      ...
      if (value[0] == 0x05) {
        ...
        let contexts = updateShortCutKey(i)
...
function updateShortCutKey(index) {
  let addr = MouseEepromAddr.ShortCutKey + 0x20 * index
  let count = flashData[addr]
  let contexts = []
  for (let i = 0; i < (count / 2); i++) {
    let type = flashData[addr + i * 0x03 + 1] & 0x0F
    let value = (flashData[addr + i * 0x03 + 3] << 8)
...
function updateMacro(index) {
  let addr = MouseEepromAddr.Macro + 0x180 * index

  let nameLen = flashData[addr]
  let contextLen = flashData[addr + 0x1F]
  var context
  if ((nameLen <= 30) && (nameLen > 0)
    ...
    for (var i = 0; i < contextLen; i++) {
      let tmp = flashData[addr + 0x20 + i * 5]

      let status = tmp >> 6
      status = status === 2 ? 0 : 1
      let type = tmp & 0x0F
      let value = (flashData[addr + 0x20 + i * 5 + 2] << 8)
      ...
      let delay = (flashData[addr + 0x20 + i * 5 + 3] << 8)
...
async function setDPIValue(index, value) {
  ...
  if (flag == true) {
    let addr = MouseEepromAddr.DPIValue + index * 4
    let data = Uint8Array.of(0x00, 0x00, 0x00, 0x00)
    let low = (value / 50 - 1) & 0xFF
    data[0] = low
    data[1] = low
    let high = (value / 50 - 1) >> 8
...
async function setDPIColor(index, value) {
  ...
  if (flag == true) {
    let addr = MouseEepromAddr.DPIColor + index * 4
...
async function setDPILightBri(value) {
  ...
  if (flag == true) {
    let bri = IndexToDPILightBri(value)
...
async function setKeyFunction(index, value) {
  ...
  if (flag == true) {
    let addr = MouseEepromAddr.KeyFunction + index * 4
...
async function setMultimedia(index, multimedia) {
  ...
  if (flag == true) {
    let addr = MouseEepromAddr.ShortCutKey + index * 0x20
    let value = []
...
async function getShortCutKey(index) {
  ...
  let count = flashData[MouseEepromAddr.ShortCutKey + index * 0x20]
  if (count > 2) {
    let cnt = ((count * 4 - 10) / 10) + 1
    cnt = Math.floor(cnt)
    console.log('getShortCutKey:', cnt)
    for (let i = 0; i < cnt; i++) {
      let tmp = (cnt - i + 2) * 4 - 10
      let len = tmp > 10 ? 10 : tmp
...
async function setMacroName(index, name) {
  ...
  if (flag == true) {
    ...
    let addr = MouseEepromAddr.Macro + index * 0x180
    await setArrayEeprom(addr, value)
  }
}
...
async function setMacroContext(index, contexts) {
  ...
  if (flag == true) {
    ...
    for (let i = 0; i < contexts.length; i++) {
      ...
      let para = (status << 6) + contexts[i].type
      value.push(para)
    ...
    let addr = MouseEepromAddr.Macro + index * 0x180 + 0x1F
    await setArrayEeprom(addr, value)
  }
}
...
async function restoreMacro(index) {
  let addr = MouseEepromAddr.Macro + index * 0x180
  let value = Uint8Array.from({ length: 0x180 }, () => 0)
...
async function getMacroContext(index) {
  ...
  let count = flashData[addr]
  if (count > 2) {
    let cnt = ((count * 5 - 10) / 10) + 1
    cnt = Math.floor(cnt)
    for (let i = 0; i < cnt; i++) {
      let tmp = (cnt - i + 2) * 5 - 10
      let len = tmp > 10 ? 10 : tmp
